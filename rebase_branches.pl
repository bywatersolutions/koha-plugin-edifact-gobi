...
say "\nNot koha-plugin-edifact-gobi, exiting" && exit 0 unless $repo eq "koha-plugin-edifact-gobi";
...
foreach my $repo (@repos) {
    next if $repo eq 'koha-plugin-edifact-gobi';
    next if $repo eq 'koha-plugin-edifact-gobi-docs';
