package Koha::Plugin::Com::ByWaterSolutions::EdifactGobi::Edifact;
...
use Koha::Plugin::Com::ByWaterSolutions::EdifactGobi::Edifact::Segment;
use Koha::Plugin::Com::ByWaterSolutions::EdifactGobi::Edifact::Message;
...
sub message_array {
    ...
    foreach my $seg ( @{ $self->{transmission} } ) {
        if ( $seg->tag eq 'UNH' ) {
            ...
            $in_msg = 0;
            if ( @{$msg} ) {
                push @{$msg_arr}, Koha::Plugin::Com::ByWaterSolutions::EdifactGobi::Edifact::Message->new($msg);
                $msg = [];
            }
        }
        elsif ($in_msg) {
...
(?>         # dont backtrack into this group
    ...
    while ( $raw =~ /($re)/g ) {
        ...
        push @segmented, Koha::Plugin::Com::ByWaterSolutions::EdifactGobi::Edifact::Segment->new( { seg_string => $cap } );
    }
    return \@segmented;
}
